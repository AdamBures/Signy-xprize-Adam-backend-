...
const APP_ROUTES = new Set(['home', 'dashboard', 'library', 'words', 'progress', 'profile', 'practice', 'translate']);
...
function lessonCards(extra='', limit=lessonGroups.length){
  ...
  sections.forEach((sec, secIdx) => {
    ...
    sec.lessons.forEach((lg, i) => {
      const globalIdx = secIdx * 10 + i;
      ...
      const alignment = i % 3 === 0 ? 'align-left' : (i % 3 === 1 ? 'align-center' : 'align-right');
      ...
      html += `
        <div class="tree-node-wrapper ${alignment} ${opacityClass}">
          <article class="lesson-card tree-card" tabindex="${isLocked ? '-1' : '0'}" role="button" data-lesson-group="${lg.id}" ${isLocked ? 'style="pointer-events:none;opacity:0.65;filter:grayscale(1);"' : ''}>
            <div class="lesson-visual">
          ...
          ${i < sec.lessons.length - 1 ? `<div class="tree-connector"></div>` : ''}
        </div>
      `;
    });
  });
  ...
  return `
    <header class="app-top">
      ...
      <div class="user-row">
        ...
        <div class="level-badge" title="XP: ${userXpInLevel}/100" style="display:inline-flex;align-items:center;background:var(--brand);color:#fff;padding:6px 12px;border-radius:99px;font-weight:bold;font-size:13px;height:36px;">
          Lvl ${userLevel}
        </div>
        ...
        <section class="modal onboarding-modal" role="dialog" aria-modal="true" aria-labelledby="modalTitle" style="width:min(580px,100%);">
          <div id="onboardingWizardContent">
            ...
            <div style="display:flex;flex-direction:column;gap:12px;margin:20px 0;">
              ...
              <button class="btn btn-lime" id="onboardingHasKnowledgeBtn" style="padding:16px;text-align:left;display:block;width:100%;color:var(--ink);">
        ...
        <section class="modal shop-modal" role="dialog" aria-modal="true" aria-labelledby="modalTitle" style="width:min(480px,100%);">
          ...
          <div style="border-top:1px solid var(--line);padding-top:15px;margin-top:15px;">
            <div style="display:flex;justify-content:between;align-items:center;gap:12px;background:var(--paper);border:1px solid var(--line);border-radius:14px;padding:14px;">
              ...
              <button class="btn btn-lime small-btn" id="buyPremiumShopBtn" style="padding:10px 16px;font-weight:bold;color:var(--ink);display:inline-flex;align-items:center;gap:4px;">
                500 ${icons.coin}
              </button>
            </div>
          </div>
...
function render(){
  stopCamera();
  const views = { home, dashboard, library, words: wordsPage, progress:progressPage, profile:profilePage, practice, translate:translatePage };
  ...
  function showOnboardingStepGoal() {
    content.innerHTML = `
      ...
      <div style="display:flex;flex-direction:column;gap:10px;margin:20px 0;">
        <button class="btn btn-ghost goal-choice-btn active" data-goal="5" style="width:100%;text-align:left;padding:12px 16px;">
          <strong>${I18n.t('Regular')}</strong> (5 ${I18n.t('minutes a day')})
        </button>
        <button class="btn btn-ghost goal-choice-btn" data-goal="15" style="width:100%;text-align:left;padding:12px 16px;">
          <strong>${I18n.t('Active')}</strong> (15 ${I18n.t('minutes a day')})
        </button>
        <button class="btn btn-ghost goal-choice-btn" data-goal="30" style="width:100%;text-align:left;padding:12px 16px;">
...
async function saveUserSettings(e){
  ...
  try{
    const updated=await Api.updateProfile({name,email,avatar,new_password,country,pronouns,skin_tone});
